...
The following diagram illustrates how data flows from devices through LSL to applications:

\begin{center}
\begin{tikzpicture}[
    node distance=1.5cm and 2cm,
    device/.style={rectangle, draw, fill=blue!20, minimum width=2.5cm, minimum height=1cm, align=center},
    lsl/.style={rectangle, draw, fill=orange!20, minimum width=10cm, minimum height=1cm, align=center},
    app/.style={rectangle, draw, fill=green!20, minimum width=2.5cm, minimum height=1cm, align=center},
    arrow/.style={-{Stealth}, thick}
]

% Devices (top row)
\node[device] (bioradio) {BioRadio\\(Device)};
\node[device, right=of bioradio] (myo) {Myo\\(Armband)};
\node[device, right=of myo] (markers) {Markers\\(Software)};

% LSL layer (middle)
\node[lsl, below=1.5cm of myo] (lsl) {Lab Streaming Layer (Network)};

% Applications (bottom row)
\node[app, below=1.5cm of lsl, xshift=-3cm] (viz) {Visualizer};
\node[app, right=of viz] (rec) {Recorder};
\node[app, right=of rec] (analysis) {Analysis};

% Arrows from devices to LSL
\draw[arrow] (bioradio) -- (bioradio |- lsl.north);
\draw[arrow] (myo) -- (lsl);
\draw[arrow] (markers) -- (markers |- lsl.north);

% Arrows from LSL to applications
\draw[arrow] (viz |- lsl.south) -- (viz);
\draw[arrow] (lsl) -- (rec);
\draw[arrow] (analysis |- lsl.south) -- (analysis);
...
The Myo Armband provides 8 channels of surface EMG (sEMG) at 200 Hz.

\subsection{Software Setup}

The lab uses \texttt{pyomyo}, which communicates directly via Bluetooth---no SDK or MyoConnect needed!

\begin{enumerate}
    \item Install pyomyo from GitHub:
    \begin{verbatim}
pip install git+https://github.com/PerlinWarp/pyomyo.git
    \end{verbatim}
    \item \textbf{IMPORTANT:} Close MyoConnect if it is running! pyomyo and MyoConnect cannot both access the Myo at the same time.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Check that BioCapture is running (for BioRadio)
    \item Try increasing timeout: \texttt{discover\_streams(timeout=5.0)}
\end{itemize}

\subsection{Myo Connection Issues}
\begin{itemize}
    \item \textbf{Close MyoConnect first!} pyomyo and MyoConnect cannot both access the Myo
    \item Make sure the Myo is charged and powered on
    \item The Myo should vibrate when pyomyo connects
    \item Test with mock data: \texttt{python src/myo\_interface.py --mock}
\end{itemize}
